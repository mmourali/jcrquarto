...
local function build_article_header()
  ...
  local has_note = saved_meta.author_note ~= ""
  local has_editor = saved_meta.editor ~= ""
  local has_assoc = saved_meta.assoc_editor ~= ""
  local has_date = saved_meta.pub_date ~= ""

  if has_note or has_editor or has_assoc or has_date then
    ...
    if has_editor then
      table.insert(lines, "\\par\\vskip 0.4\\baselineskip\\textit{Editor: " .. saved_meta.editor .. "}")
    end
    if has_assoc then
      table.insert(lines, "\\par\\textit{Associate Editor: " .. saved_meta.assoc_editor .. "}")
    end
    if has_date then
      table.insert(lines, "\\par\\textit{Advance Access publication " .. saved_meta.pub_date .. "}")
    end
